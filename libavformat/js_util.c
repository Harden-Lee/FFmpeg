...
static uint8_t *base64_decode(uint8_t *bindata, int32_t inlen, uint8_t **out, int32_t *outlen)
{
    int32_t _outlen = *outlen;
    uint8_t *_out = NULL;
...
static void sha1Update(SHA1_CTX* context, const uint8_t* data, const size_t len)
{
    size_t i, j;
...
static void sha1Final(SHA1_CTX* context, uint8_t digest[SHA1_DIGEST_SIZE])
{
    uint32_t i;
...
static uint8_t *digest_to_hex(const uint8_t digest[SHA1_DIGEST_SIZE], uint8_t *output)
{
    int i = 0;
    int len = 0;
    char *c = (char *) output;
...
static uint8_t *sha1BuffHex(const void *buff, size_t lenth, uint8_t *out)
{
...
static uint8_t *sha1File(char *filename, uint8_t* out)
{
    uint8_t digest[20] = {0};
    ...
    sha1Init(&mdContext);
    while ((bytes = fread(data, (size_t)1, 1024, inFile)) != 0)
    {
...
//    hints.ai_flags = AI_ALL | AI_CANONNAME | AI_PASSIVE;
    ...
    if (iret != 0)
    {
        av_log(NULL, AV_LOG_ERROR, "getaddrinfo: %s\n", gai_strerror(iret));
        return -1;
    }
    else
    {
...
static int checkConnect(int iSocket) {
    struct pollfd fd;
    int ret = 0;
    socklen_t len = 0;
    ...
    if(ret != 0) {
        av_log(NULL, AV_LOG_ERROR, "socket %d connect failed: %s\n",
                 iSocket, strerror (ret));
        return -1;
    }
    av_log(NULL, AV_LOG_ERROR,"CheckConnect %d\n", ret);
    return 0;
}
...
int32_t ut_connect(const char *hostname, uint16_t port)
{
    ...
    av_log(NULL, AV_LOG_ERROR,"connecting to %s %d\n", hostname, port);
    if (connect(fd, (struct sockaddr *) &addr_remote, addr_len) < 0)
    {
        if(checkConnect(fd) == -1){
            return -1;
        }
    }
    av_log(NULL, AV_LOG_ERROR,"connected to %s %d\n", hostname, port);
    return fd;
}

uint64_t ntohlll(uint64_t val)
{
    return (((uint64_t) htonl((int) ((val << 32) >> 32))) << 32) | (uint32_t) htonl((int) (val >> 32));
}
//
uint64_t htonlll(uint64_t val)
{
    return (((uint64_t) htonl((int) ((val << 32) >> 32))) << 32) | (uint32_t) htonl((int) (val >> 32));
}
