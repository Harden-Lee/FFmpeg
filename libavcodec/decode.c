...
static int ff_get_nal_units_type(const uint8_t * const data)
{
    return   data[4] & 0x1f;
}

static int ff_avpacket_is_sei(const AVPacket* pkt)
{
    if (pkt->timestamp) {
        return 1;
    }
...
static int parser_sei_pkt(AVPacket *pkt,char *sei_content)
{
    if (ff_avpacket_is_sei(pkt)) {
       if (pkt->timestamp > 0) {
           memset(sei_content, 0, 24);
           snprintf(sei_content,24,"%"PRIu64"",pkt->timestamp);
           av_log(NULL, AV_LOG_INFO, "parser_sei_pkt ======== %s\n",sei_content);
        }else{
            const uint8_t* p_sei = pkt->data + 23;
            const int len = p_sei[1] & 0xff;
            memset(sei_content, 0, 24);
            memcpy(sei_content, p_sei, len > 24 ? 24 : len);
            av_log(NULL, AV_LOG_INFO, "parser_sei_pkt ======== %s\n",sei_content);
        }
        return 1;
    }
...
static int decode_simple_internal(AVCodecContext *avctx, AVFrame *frame)
{
    ...
    if (strlen(frame->timestamp) <= 0) {
        parser_sei_pkt(pkt,frame->timestamp);
    }
...
//    av_log(NULL, AV_LOG_INFO, "avcodec_send_packet ======== avci->buffer_pkt.timestamp == %llu \n",avci->buffer_pkt->timestamp);
